...
function creatureSayCallback(cid, type, msg)
	...
	if msgcontains(msg, "permission") then
		npcHandler:say("You want to acquire permissions for the inquisition quest? If you want to say {inquisition} or {rules}...", cid)
	...
	elseif npcHandler.topic[cid] == 1 then
		...
		elseif msgcontains(msg, 'prices') then
			npcHandler:say("A great inquisition quest cost 500000 gold coins.", cid)
			npcHandler.topic[cid] = 0
		end	
	elseif msgcontains(msg, "inquisition") and npcHandler.topic[cid] == 2 then
		npcHandler:say("You really want to pay 500000 gold coins in the permissions inquisition quest, knowing you back there dead?", cid)
		npcHandler.topic[cid] = 3
	elseif msgcontains(msg, "yes") and npcHandler.topic[cid] == 3 then
		local player = Player(cid)
		if player:getStorageValue(Storage.inquisition) == -1 then
			if not player:removeMoney(500000) then
