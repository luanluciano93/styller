...
function onThink(interval, lastExecution)
	local orderQuery = db.storeQuery([[
		...
		WHERE `shop`.`type` IN(1,5,6,7,8,9,10)
		GROUP BY `shop`.`id`
	]])
		...
		repeat 
			...
			if player ~= nil then
				...
				if tile ~= nil and tile:hasFlag(TILESTATE_PROTECTIONZONE) then
					...
					if orderType == 1 or orderType == 7 or orderType == 8 or orderType == 9 or orderType == 10 then
						served = true
						...
						if player:getFreeCapacity() >= itemType:getWeight(orderCount) then
							...
							if backpack ~= nil and backpack:getEmptySlots(false) >= needslots then
								...
								local item = player:addItem(orderItemId, orderCount)
